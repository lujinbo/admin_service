...
class Lib_Mysql
{
	...
	public function connect($dbhost,$dbuser,$dbpwd,$dbname,$pconnect=0,$charset='UTF8')
	{
		if($pconnect == 0) {
			...
			//$this->linkId = mysql_connect($dbhost,$dbuser,$dbpwd,$dbname);
...
;			if(!$this->linkId) $this->errorLog("数据库连接失败");
	...
	public function query($strsql)
	{
        try{
            ...
            $sqlData = mysql_query($strsql,$this->linkId);
        }catch (\Exception $e){
            $sqlData = false;
     ...
     * @return array|bool
     */
	public function getOne($strsql,$type=MYSQL_ASSOC)
	{
		$data = $this->query($strsql);
		if(empty($data))
		    return false;
		$ret = mysql_fetch_array($data,$type);
		return $ret;	
	}
     ...
     * @return array|bool
     */
	public function getAll($strsql,$type=MYSQL_ASSOC)
	{
		$data = $this->query($strsql);
		if(empty($data))
		    return false;
		$i = 0;
		$ret = array();
		while ($row = mysql_fetch_array($data,$type)) {
			$ret[$i] = $row;
			$i++;
		}
		return $ret;
